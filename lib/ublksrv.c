...
#define UBLK_OP_EVENTFD    0
#define UBLK_OP_EPOLLFD    1
...
static inline int __ublksrv_queue_event(struct _ublksrv_queue *q)
{
	if (q->efd >= 0) {
		struct io_uring_sqe *sqe;
		__u64 user_data = build_internal_data(UBLK_OP_EVENTFD);
...
static int ublksrv_setup_epollfd(struct _ublksrv_queue *q)
{
	...
	struct io_uring_sqe *sqe;
	__u64 user_data = build_internal_data(UBLK_OP_EPOLLFD);
